...
from networks.Windows_Shift import Relative_Position_Layer, Block, window_partition, window_reverse
from networks.TC_MoA import ConvFusionLayer, BiMixtureOfAdapters
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3,
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, 197, decoder_embed_dim),
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if (i + self.tau_shift_value) % self.interval_tau == self.interval_tau - 1:
                ...
                with torch.cuda.amp.autocast(enabled=False):
                    x_MoA, t_MoA, x_prompt, t_prompt, aux_MoA = self.blocks_MoA[block_group_index](x, t)
    ...
    def forward_decoder(self, x, t, encoder_info):
        # embed tokens
        ...
        for i, blk in enumerate(self.decoder_blocks):
            ...
            x = blk(x, relative_position, have_shift, H, W, window_size, shift_size)
            t = blk(t, relative_position, have_shift, H, W, window_size, shift_size)
            if i % self.interval_tau == self.interval_tau-1:
                # print(f'{i} th blocks')
                block_group_index = i // self.interval_tau
                with torch.cuda.amp.autocast(enabled=False):
                    x_MoA, t_MoA, x_prompt, t_prompt, aux_MoA = self.decoder_blocks_MoA[block_group_index](x, t)
                    aux_loss = aux_loss + aux_MoA
    ...
    def getPromptLoss(self, prompt):
        ...
        prompt_loss = 0.75 * self.mse(prompt_rgb + prompt_t, prompt_gt) + 0.25 * self.mse(de_prompt_rgb + de_prompt_t,
                                                                                          de_prompt_gt)
    ...
    def forward_loss_split(self, img_rgb, img_t, pred, prompt, epoch, aux_loss):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        prompt: [# B 1 H W],
    ...
    def forward(self, img_rgb, img_t, epoch):
        ...
        pred, de_prompt_rgb, de_prompt_t, aux_loss = self.forward_decoder(latent, t, encoder_info)  # [N, L, p*p*3]
        with torch.cuda.amp.autocast(enabled=False):
...
mae_vit_large_patch16 = mae_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocksq
mae_vit_huge_patch14 = mae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks

def init_model(model_type='mae_vit_base_patch16'):
    # models_mae = MaskedAutoencoderViT()
    print("Model Type: mae_vit_base_patch16")
    # Initialize the model and load the parameters
    model = mae_vit_base_patch16(config=None)
    model = load_pretrained_weights(model, weights_path='/root/nas-public-linkdata/dataset/other/mae_pretrain_vit_base.pth')

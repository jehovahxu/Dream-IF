...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_loss_taskMFF(self, img_rgb, img_t, pred, prompt, epoch, aux_loss):
        ...
        if epoch < self.warmup_epochs:
            ...
            loss = 20 * (max_grad_loss + pixel_loss) + 20 * prompt_loss + ssim_loss + aux_loss
            loss = {"loss": loss,
                    "ssim_loss": ssim_loss,
                    "max_grad_loss": max_grad_loss,
                    "aux_loss": aux_loss,
    ...
    def forward(self, img_rgb, img_t, task_index=0, epoch=0):
        ...
        pred, de_prompt_rgb, de_prompt_t, aux_loss = self.forward_decoder(latent, t, task_index,
                                                                          encoder_info)  # [N, L, p*p*3]

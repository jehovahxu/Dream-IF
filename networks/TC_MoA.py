...
from networks.MMOE import MMoE, MMoE_task
...
class BiMixtureOfAdapters(nn.Module):
    ...
    def __init__(
        ...
        self.MoA_task_specific = MMoE_task(dim // 4, dim // 4, 3, dim // 32, noisy_gating=True, k=1, task_num=3)
        self.MoA_generative = MMoE(dim // 4, dim // 4, 4, dim // 32, noisy_gating=True, k=2, task_num=3)

        self.merge = nn.Linear(dim // 2, dim // 4, bias=False)
        # print()
    ...
    def forward(self, x, t, task_index=0):
        ...
        B, N, C = x.shape
        y = self.norm1(y)
        self.reduction = self.dimReduction(y)
        y = self.reduction
        y = self.norm2(y)
        y = y.view(B * N, C // 4)
        # y1, aux_loss = self.MoA_generative(y)
        y2, aux_loss_2 = self.MoA_task_specific(y, task_index)
        y1, aux_loss_1 = self.MoA_generative(y)
        aux_loss = aux_loss_1 + aux_loss_2
        # import pdb;pdb.set_trace()
        y = self.merge(torch.cat([y1, y2], -1))
        y = y.view(B, N, C // 4)
